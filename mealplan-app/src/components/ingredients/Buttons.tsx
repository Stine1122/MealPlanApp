...
export function DeleteButtonList({ setList, str }: ListProps & { str: string }) {
    return (
        <button
            className="font-headline self-center cursor-pointer text-amber-50/80 bg-red-700/80 hover:bg-red-600 p-2 rounded-2xl mt-3"
            onClick={() => {
                setList([])
            }}
        >
...
export function DeleteButton({ list, setList, item }: ListProps & { item: string }) {
    return (
